...
- **Resume Update**: Updated the professional resume (`Guanyu_resume.pdf`) with the latest version.

## [0.3.3] - 2025-10-16

### Fixed

- **Navbar Active Link**: The navbar now correctly highlights the "Projects" and "Blogs" links when viewing individual project or blog pages.
- **Navbar Color**: Reverted the navbar background color to the original darker shade to improve contrast and visibility.

## [0.3.2] - 2025-10-11

### Changed

- **Starfield Background**: Iteratively refined the appearance of the homepage's animated starfield. The particle shapes were changed from the default squares to circles for a softer, more star-like appearance.
...
- **Advanced Search Functionality**: Implemented a new site-wide search feature accessible via a header icon or keyboard shortcut (Cmd+K/Ctrl+K).
  ...
  - **Full-Text Search**: The search engine indexes the full content of all blogs and projects, as well as static pages like "About" and "Resume".
...
- Enhanced search functionality in `src/components/SearchProvider.tsx` to include `author`, `date`, and `location` fields in FlexSearch queries, display these details in search results, and handle various date query formats.
...
- Set the background for Apple touch icons to transparent, allowing the iOS operating system to automatically apply the appropriate background based on the user's theme.
...
- Reverted the project to commit `0997726` to restore a previous state. This action was taken to undo recent changes that were not desired.
...
- Integrated Waline comment system into blog and project detail pages, using a shared backend.
